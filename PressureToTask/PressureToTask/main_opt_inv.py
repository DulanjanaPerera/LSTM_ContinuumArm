...
def objective(trial):
    ...
    lstm_hidden = trial.suggest_int('lstm_hidden', 8, 256)
    n_fc = trial.suggest_int('n_fc', 1, 6)
    fc_hidden = [trial.suggest_int(f'fc_hidden_{i}', 8, 256) for i in range(n_fc)]
    lr = trial.suggest_float('lr', 0.00001, 1e-1, log=True)
...
study = optuna.create_study(direction='minimize', sampler=TPESampler(seed=40))
...
torch.save(best_model.state_dict(), "best_model_inv_task9.pt")
...
plt.tight_layout()
plt.savefig("prediction_plot_opt_inv_test9.png", dpi=300, bbox_inches='tight')
plt.close()
